...
stop() ->
    gen_server:cast(interpreter, stop).
...
handle_cast({notify, torrent_filepath, {_,Path}},_LoopData) ->
    Result = create_record(Path),
    io:format("~p~n", [Result#torrent.announce]),
    gen_server:cast(msg_controller, {notify, torrent_info, {Result#torrent.id, Result}}),
    {noreply,Result}.
