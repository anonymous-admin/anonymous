...
create_record(FileName) ->
    ...
    File_length = get_file_length(Parsed_info, 0),
...
%% @end
%%--------------------------------------------------------------------
get_file_length({info, Info}, FileSize) ->
    {_, Size} = get_files({info, Info}, FileSize),
    Size.
...
%% @end
%%--------------------------------------------------------------------
files_interpreter([],Size) ->    
    {[],Size};
files_interpreter([H|T],Size) ->
    {dict, Info} = H,
    Length = dict:fetch(<<"length">>, Info),
    FileSize = Size + Length,
    {list, Path} = dict:fetch(<<"path">>, Info),
    {[[Path, Length]|files_interpreter(T,FileSize)], FileSize}.
